...
@section('content')
  ...
  <div class="container-fluid mt-4 mb-4">
    <h2 class="mb-4">Thêm điểm ảnh</h2>
        ...
        <div class="col-md-6 mb-3">
          ...
          <select name="paronama_id" id="paronama_id" class="form-select @error('paronama_id') is-invalid @enderror"
            ...
            @foreach ($paronamas as $paronama)
              <option value="{{ $paronama->id }}"
                {{ isset($location) && $location->paronama_id == $paronama->id ? 'selected' : '' }}>
                {{ $paronama->name }}
              </option>
            @endforeach
          </select>
      ...
      <div class="row">
        ...
        <div class="col-md-6 mb-3">
          <div class="file_sound">
            ...
            <input type="file" name="voice_en" id="voice_en" class="form-control @error('voice') is-invalid @enderror"
              accept="audio/*">
        ...
        <div class="col-md-12 mb-3">
          ...
          <select name="next_location_id" id="next_location_id"
            ...
            @foreach ($locations as $loc)
              @if (!isset($location) || $location->id != $loc->id)
                <option value="{{ $loc->id }}"
                  {{ isset($location) && $location->next_location_id == $loc->id ? 'selected' : '' }}>
                  {{ $loc->name }}
                </option>
              @endif
            @endforeach
        ...
        <div class="col-md-12 mb-3">
          <div class="form-check form-switch">
            <input type="checkbox" name="status" id="status"
              ...
              {{ isset($location) && $location->status ? 'checked' : '' }}>
